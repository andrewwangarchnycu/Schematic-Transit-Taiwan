...
function formatEstimate(item, t) {
  if (item.StopStatus === 1) return t("estimateNotDeparted");
  if (item.StopStatus === 4) return t("estimateNoService");
  if (item.StopStatus !== 0 || item.EstimateTime == null) return t("estimateNoInfo");
  if (item.EstimateTime <= 90) return t("estimateArriving");
  return t("estimateMinutes", { minutes: Math.round(item.EstimateTime / 60) });
}
...
export default function StopDetail({ city, station, onBack }) {
  ...
  useEffect(() => {
    ...
    function load() {
      getStopEta(city, station.StationID)
        .then((data) => {
          if (!cancelled) {
            setEtas(data);
            setError(null);
  ...
  const sorted = [...etas].sort((a, b) => (a.EstimateTime ?? 1e9) - (b.EstimateTime ?? 1e9));
        ...
        {sorted.map((item, idx) => {
          ...
          return (
            <li key={`${item.RouteID}-${item.Direction}-${idx}`} className="eta-row">
              ...
              <span className={`eta-time ${item.EstimateTime != null && item.EstimateTime <= 90 ? "eta-soon" : ""}`}>
